...
import {
  ...
  Grid3X3, List, MapPin, Truck, Shield, Leaf
} from 'lucide-react';
...
const UserProductView = () => {
  ...
  const ProductCard = ({ product }) => (
    <div className={`bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all group ${viewMode === 'list' ? 'flex' : ''
      }`}>
        ...
        <div className={`${viewMode === 'list' ? 'flex justify-between items-start' : ''}`}>
          ...
          <div className={`${viewMode === 'list' ? 'text-right' : ''}`}>
            <div className="flex items-center gap-2 mb-3">
              <span className="text-2xl font-bold text-green-600">GH₵{product.price}</span>
            </div>
